...
public class OfflineExercises {
	...
	public String getBert(String input) {
		input = "_"+ input.toLowerCase() +"_";
		String findbert [] = input.split("bert");
		String output = " ";
			if(findbert.length < 3) {
				return "";
			}
			for (int i = 0; i < findbert[1].length(); i++) {
					output= findbert[1].charAt(i) + output;
				}
...
//		input.toLowerCase().endsWith("dev");
	...
	public int simpleRecursion(int input) {
		...
		if(input == 0) {
			...
			return 2 + simpleRecursion(input -1);
		}
	}
